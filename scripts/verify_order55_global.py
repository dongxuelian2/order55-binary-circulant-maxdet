"""Independent order-55 certificate audit, with deterministic generator replay.

No heuristic search or production certificate-builder module is imported.
Use --full-lifts to rerun every lift with the different 6+5 column split.
"""
...
def audit(folder,full):
    ...
    # Verify dyadic cosine enclosures independently with rational Machin/Taylor.
    def arctan(q):
    ...
    for r,(low,high) in enumerate(cos):
        x=(pilo+pihi)*r/55
        y=sum((-1)**j*x**(2*j)/factorial(2*j) for j in range(43))
        error=x**86/factorial(86)+(pihi-pilo)*r/55
        assert F(low,1<<24)<=2*(y-error)<=2*(y+error)<=F(high,1<<24)
    # Independently derive every allowed norm threshold.
    expected=[]
    for m in (5,11):
        h=(m-1)//2;rest=27-h
    ...
    # Recompile source whose hashes are part of the certificate; replay all small
    # exhaustive generators. Lift replay is optional because it is the costly part.
    with tempfile.TemporaryDirectory(prefix='order55-audit-') as tmp:
        ...
        for k in sorted(limits):
            for m in (5,11):
                ...
                for row in lines(stored):
                    ...
                    # Independent exact integer resultant for each surviving norm.
        ...
        for k in sorted(limits):
            # Enumerate complete sorted correlation profiles independently.
            multisets=set()
        ...
        for entry in entries:
            ...
            s2=F(55*(k*k+2*sq)-k**4,2)
            bound=(55-k)*product_bound(F(k*(55-k),2),s2,27,cap);assert int(bound)==entry['upper_floor']
            assert (2*(55-k))**2*s2**27<(P[0]*P[1])**2*27**27
            A=F(isqrt(int(27*s2))+1)+F(27*sum(abs(v) for v in ds)*max(b-a for a,b in cos),1<<24)
            pref=max([F(1)]+[(A/(16*h))**h for h in range(1,28)])
            ...
            assert digest(replay)==digest(stored)
            fresh=load(ra);old=entry['audit'];assert fresh['ordered_profiles']==count
        ...
        for idx,row in enumerate(coverage):
            ...
            assert [list(r) for r in rs]==row['row_profiles'] and [list(s) for s in ss]==row['column_profiles']
            assert len(rs)*len(ss)==row['tasks']
        ...
        for part in parts:
            i=part['part'];inp=folder/f'lift_part{i}.txt';auditpath=folder/f'lift_part{i}_audit.jsonl';words=folder/f'lift_part{i}_words.txt'
            expected_ids=set()
            ...
            audits=lines(auditpath);assert len(audits)==len(expected_ids) and {a['task'] for a in audits}==expected_ids
            assert sum(a['joined_words'] for a in audits)==part['joined_words'];joined+=part['joined_words']
            actual=words.read_text().splitlines();assert len(actual)==part['solutions']==sum(a['solutions'] for a in audits)
            for line in actual:
                task,w=line.split();assert task in expected_ids and len(w)==55 and set(w)<={'0','1'}
                values=tasks[task];assert fold(w,5)==values[:5] and fold(w,11)==values[5:16] and corr(w)==values[16:]
                high=''.join('1' if b=='0' else '0' for b in w);value=det(high);low=det(w)
                assert low*(55-w.count('1'))==value*w.count('1')
                assert value==int(coverage[int(task.split('_')[0])]['target']['absolute_profile_product']);wordrecords.append((high,value))
        ...
        if full:
            def replay_lift(part):
                ...
                assert set(freshwords.read_text().splitlines())==set((folder/f'lift_part{i}_words.txt').read_text().splitlines())
                old={r['task']:r for r in lines(folder/f'lift_part{i}_audit.jsonl')};new=lines(freshaudit)
                assert len(new)==len(old)
                for r in new:assert (r['joined_words'],r['solutions'])==(old[r['task']]['joined_words'],old[r['task']]['solutions'])
                print('INDEPENDENT 6+5 LIFT SPLIT PASS',i,flush=True)
            with ThreadPoolExecutor(max_workers=8) as pool:list(pool.map(replay_lift,parts))
    assert max(v for w,v in wordrecords)==maximum
    classes={canonical(w) for w,v in wordrecords if v==maximum};winner=load(folder/'winner.json')
    assert classes=={r['word'] for r in winner['classes']} and len(classes)==g['affine_class_count']
    count=0
    for row in winner['classes']:
        w=row['word'];assert det(w)==maximum and w.count('1')==row['weight']
        assert row['residues']==[maximum%p for p in P]
        ...
        x=sp.Symbol('x');f=sum(int(b)*x**i for i,b in enumerate(w));norms=[int(sp.resultant(f,sp.cyclotomic_poly(m,x),x)) for m in (5,11,55)]
        assert list(map(str,norms))==row['norms'] and row['weight']*prod(norms)==maximum
        orbit={''.join(w[(u*i+t)%55] for i in range(55)) for u in range(1,55) if gcd(u,55)==1 for t in range(55)}
        assert len(orbit)==row['orbit_size'] and row['stabilizer_size']*len(orbit)==2200
    ...
    print('ORDER 55 GLOBAL CERTIFICATE')
    print('winner =',winner['classes'][0]['word']);print('weight =',winner['classes'][0]['weight']);print('determinant =',maximum)
    print('affine_class_count =',len(classes));print('maximizing_words =',count);print('covered_words =',2**55);print('GLOBAL MAXIMUM: PASS')
